  ...
  <div class="banner-list">
    <div class="search-area">
      ...
      <div class="right-button-wrap">
        <a-button type="primary" @click="showModal = true">修改汇率</a-button>
      </div>
    </div>
    ...
    <!-- 修改汇率 -->
    <rate-change v-if="showModal" :visible.sync="showModal" />
  </div>
</template>
...
import columns from './table-utils/rateList'
...
export default {
  ...
  methods: {
    ...
    getList(query) {
      ...
      this['SET_CLICK'](true)
      this.$apis
        .get_manage_rate_list(params)
        .then(res => {
          this.pagination.total = res.total
          this.dataList = [...res.data]

          // this.$nextTick(() => {
          //   let trNodes = document.getElementsByClassName('ant-table-tbody')[0].children
          //   for (let item of trNodes) {
          //     const rowKey = item.getAttribute('data-row-key')
          //     if (rowKey < 7) {
          //       item.style.backgroundColor = '#ffcc33'
          //     }
          //   }
          // })
